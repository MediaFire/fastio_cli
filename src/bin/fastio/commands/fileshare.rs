...
fn map_fileshare_error(err: CliError, ctx: &'static str, op: FsOp) -> anyhow::Error {
    ...
    let mapped: Option<(String, FsHint)> = match api.code {
        ...
        // so keying on the code alone is op-independent
...
async fn upload_writeback(
    ...
    // Reject a BLANK precondition before anything else, including the confirm
    // gate: an empty `--if-version` is not "no precondition", it is a base the
    // server could never check. `api/upload.rs` sends the field whenever it is
    // `Some`, so a blank would otherwise reach the wire and the outcome would
    // depend on server-side input validation the upload reference does not
    // document (the storage reference rejects it; the upload page is silent).
    // Refusing locally makes the CLI fail-safe under EITHER server behaviour.
    // Shared with `files update` so the two surfaces cannot diverge.
    storage::reject_unsupported_if_version(if_version)?;
...
async fn single_shot_writeback(
    ...
    // The immediate response cannot carry the outcome: assembly happens AFTER
    // it returns, so a compare-and-swap rejection arrives later as
    // `assembly_failed` + `CONFLICT_VERSION_MISMATCH:{current}`. Checking only
    // this response reports EXIT 0 for a write the server then refuses —
    // measured 2026-08-24 with a stale `--if-version` on a 45-byte file, while
    // the chunked path above correctly surfaced the same conflict. Same command,
    // same flag, opposite outcome decided by file size.
    //
    // So poll to a terminal status here too, reusing the chunked path's loop
    // rather than a second copy: one definition of "what counts as done".
    if let Some(upload_id) = fileshare::extract_session(&resp)
    ...
    {
        let client = ctx.build_client()?;
        return poll_writeback_completion(&client, &upload_id, password).await;
    }
    Ok(resp)
}
...
///
/// The upload contract makes chunk ordering 1-based: the
...
async fn send_writeback_chunks(
    ...
    use std::io::Read;
    // The chunk `order` is 1-BASED per the upload contract ("first chunk is
    // `order=1`"), matching the canonical upload command's
...
mod tests {
    ...
    fn writeback_chunk_order_is_one_based() {
        ...
        // upload contract. A 0-based first chunk would be rejected server-side.
        assert_eq!(
