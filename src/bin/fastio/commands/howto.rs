...
pub async fn execute(
    ...
    // The CLI terminal surface identifies itself so the answer engine can
    // apply a per-client policy; the embedded MCP server sends its own name.
    let value = api::howto::ask(&client, question, context, surface, Some("fastio-cli"))
        .await
