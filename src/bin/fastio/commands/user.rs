...
/// secrets (they must never appear in Debug output). This mirrors the
/// secret-bearing internal `AuthCommand` / `TwoFaCommand` enums, which also
/// omit `Debug`.
#[derive(Clone)]
#[non_exhaustive]
pub enum UserCommand {
    ...
    Close {
        /// The account's own email address (the server's confirmation check).
        email_address: String,
        /// Check eligibility without closing anything.
        dryrun: bool,
    },
...
/// `token`) — they must never appear in Debug output.
#[derive(Clone)]
#[non_exhaustive]
...
pub async fn execute(command: &UserCommand, ctx: &CommandContext<'_>) -> Result<()> {
    match command {
        ...
        UserCommand::Close {
            ...
            let client = ctx.build_client()?;
            let value = api::user::close_account(&client, email_address, *dryrun)
                .await
