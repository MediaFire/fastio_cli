...
pub enum WorkspaceCommand {
    ...
    Search {
        ...
        /// Offset for pagination.
        offset: Option<u32>,
        /// `search_in` / `name_match` / `case_sensitive` (files bucket only).
...
pub async fn execute(command: &WorkspaceCommand, ctx: &CommandContext<'_>) -> Result<()> {
    match command {
        WorkspaceCommand::List {
            ...
            offset,
            archived,
        } => list(ctx, org_id.as_deref(), *limit, *offset, *archived).await,
        ...
        WorkspaceCommand::Search {
            ...
            offset,
            modes,
        } => search(ctx, workspace_id, query, *limit, *offset, modes.clone()).await,
...
async fn list(
    ...
    let value = api::workspace::list_workspaces(&client, org_id, limit, offset, archived)
        .await
...
/// Search workspace content.
///
/// Re-pointed onto the unified grouped-bucket search
