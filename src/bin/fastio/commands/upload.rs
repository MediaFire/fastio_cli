...
/// Fallback chunk size when the plan's limits cannot be read: 4 MB.
///
/// Used only when the plan's limits cannot be read: the chunk size is
/// **plan-dependent**, and the chunk `order` "must not exceed plan's chunk
/// limit" — 500 on the documented plan. A fixed 4 MB chunk therefore caps an
/// upload at 500 × 4 MB = **2 GB**, on a plan whose documented maximum file size
/// is 40 GB, and nothing detects that up front: the upload runs normally and
/// fails at chunk 501, two gigabytes in.
const DEFAULT_CHUNK_SIZE: usize = 4 * 1024 * 1024;

/// The chunking parameters resolved from `GET /upload/limits/`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct ChunkPlan {
    /// Bytes per chunk for this upload.
    chunk_size: usize,
}

/// Choose a chunk size that fits BOTH the plan's per-chunk cap and its chunk
/// count, or explain up front why the file cannot be uploaded at all.
///
/// Sizing rules, in order:
/// 1. A file larger than `limits.size` is rejected **before** a session exists,
///    rather than after uploading most of it.
/// 2. Otherwise start from the current 4 MB — small files keep today's memory
///    profile and today's retry granularity.
/// 3. Grow only as far as the chunk COUNT demands (`ceil(size / limits.chunks)`),
///    never past `limits.chunk_size`.
/// 4. If even the maximum chunk size cannot cover the file within the chunk
///    count, say so now instead of failing mid-flight.
///
/// A missing or unparseable field falls back to today's behaviour: this must
/// never make an upload that used to work start failing because a limits lookup
/// returned something unexpected.
fn plan_chunks(limits: Option<&Value>, file_size: u64) -> Result<ChunkPlan> {
    ...
    if let Some(max_size) = read_u64("size")
        && file_size > max_size
    {
        anyhow::bail!(
            "file is {} but this plan allows at most {} per upload — \
             refusing before starting rather than failing part-way through",
            format_bytes(file_size),
            format_bytes(max_size),
        );
    }

    let cap = read_u64("chunk_size")
        .and_then(|v| usize::try_from(v).ok())
        .unwrap_or(DEFAULT_CHUNK_SIZE)
        .max(1);
    let mut chunk_size = DEFAULT_CHUNK_SIZE.min(cap);

    if let Some(max_chunks) = read_u64("chunks") {
        let needed = file_size.div_ceil(max_chunks);
        if let Ok(needed) = usize::try_from(needed) {
            chunk_size = chunk_size.max(needed);
        }
        if chunk_size > cap {
            anyhow::bail!(
                "file is {} — at this plan's maximum chunk size ({}) that needs more \
                 than the {max_chunks} chunks allowed per session. Refusing before \
                 starting rather than failing at chunk {}.",
                format_bytes(file_size),
    ...
    Ok(ChunkPlan {
        chunk_size: chunk_size.min(cap).max(1),
    })
}

/// Fetch the plan's upload limits for this target, then size the chunks.
///
/// A failed lookup is NOT fatal — it falls back to the historical 4 MB. The
/// point of this call is to lift a cap, never to add a new way for an upload to
/// fail before it starts.
async fn resolve_chunk_plan(
    client: &ApiClient,
    instance_id: &str,
    folder: &str,
    file_size: u64,
    ...
    let limits = api::upload::upload_limits(
        client,
        Some("create"),
        None,
        Some(instance_id),
    ...
    .await
    .ok();
    plan_chunks(limits.as_ref(), file_size)
}
...
fn walk_tree_inputs(root: &str) -> Result<Vec<ResolvedInput>> {
    ...
    while let Some(dir) = stack.pop() {
        ...
        for entry in entries {
            ...
            // Validate here, where the path is CONSTRUCTED, so every consumer
            // of a `ResolvedInput` is covered. Validating only inside the batch
            // orchestrator would leave the non-batch upload path forwarding an
            // over-long segment, which the server accepts and then silently
            // shortens — merging two distinct folders without an error.
            if let Some(rp) = &relative_path {
                fastio_cli::api::upload::validate_relative_path(rp).map_err(|e| {
                    anyhow::anyhow!("invalid relative_path for {}: {e}", entry_path.display())
                })?;
            }
            out.push(ResolvedInput {
...
async fn upload_large_via_chunked(
    ...
    // Resolved BEFORE the session exists, so an oversize file is refused without
    // leaving a dangling session behind.
    let plan = resolve_chunk_plan(client, workspace, folder, inp.size).await?;
...
async fn upload_file(
    ...
    let plan = resolve_chunk_plan(&client, workspace, folder, file_size).await?;
    ...
    // `poll_upload_completion` returns ONLY "stored" or "complete" — every other
    // status either bails or times out — so this branch fires only on "complete",
    // which that fn already treats as terminal success. On a finished session the
    // DELETE reaps the session record and leaves the stored file alone (the web
    // client does the same on every successful upload), so this is session
    // reaping, not cancellation. Reads like data loss; is not.
...
mod tests {
    ...
    // ─── chunk planning: the 2 GB cap ──────────────────────────────────────

    /// The documented plan shape.
    fn plan_limits() -> serde_json::Value {
        serde_json::json!({"limits": {
            "chunk_size": 104_857_600_u64,   // 100 MB
            "size": 42_949_672_960_u64,      // 40 GB
            "chunks": 500_u64,
        }})
    }
    ...
    ///
    /// A fixed 4 MB chunk × the plan's 500-chunk ceiling caps an upload at 2 GB
    /// on a plan documented for 40 GB, and fails at chunk 501 after two
    /// gigabytes have already been sent. A 10 GB file must grow its chunks to
    /// fit inside 500 of them.
    #[test]
    fn a_ten_gigabyte_file_grows_its_chunks_to_fit_the_chunk_ceiling() {
        let ten_gb = 10 * 1024 * 1024 * 1024_u64;
        let plan = super::plan_chunks(Some(&plan_limits()), ten_gb).expect("must be uploadable");
        let chunks = ten_gb.div_ceil(plan.chunk_size as u64);
        assert!(
            chunks <= 500,
            "{ten_gb} bytes in {}-byte chunks needs {chunks} chunks, over the plan's 500",
            plan.chunk_size
        );
        assert!(
            plan.chunk_size <= 104_857_600,
            "must not exceed the plan's per-chunk cap: {}",
            plan.chunk_size
        );
        // The old behaviour, stated as a number so the fix cannot silently revert.
        assert!(
            plan.chunk_size > super::DEFAULT_CHUNK_SIZE,
            "4 MB chunks cannot carry 10 GB within 500 chunks"
        );
    }

    /// Small files keep TODAY's 4 MB chunking — memory profile and retry
    /// granularity are unchanged for the common case.
    #[test]
    fn a_small_file_still_uses_four_megabyte_chunks() {
        let plan = super::plan_chunks(Some(&plan_limits()), 50 * 1024 * 1024).expect("ok");
        assert_eq!(plan.chunk_size, super::DEFAULT_CHUNK_SIZE);
    }

    /// A file over the plan's maximum is refused UP FRONT, not after uploading
    /// most of it — that is the difference this change is really about.
    #[test]
    fn an_oversize_file_is_refused_before_anything_is_uploaded() {
        let err = super::plan_chunks(Some(&plan_limits()), 50 * 1024 * 1024 * 1024)
            .expect_err("50 GB exceeds the 40 GB plan maximum");
        let msg = err.to_string();
        assert!(
            msg.contains("40") && msg.contains("refusing before starting"),
            "the message must name the limit and say it stopped early: {msg}"
        );
    }

    /// A missing / unreadable limits response must fall back to today's
    /// behaviour. Lifting a cap must never introduce a NEW way for an upload
    /// that used to work to fail.
    #[test]
    fn absent_or_malformed_limits_fall_back_to_the_historical_chunk_size() {
        for limits in [
            None,
            Some(serde_json::json!({})),
            Some(serde_json::json!({"limits": {"chunk_size": 0, "chunks": 0, "size": 0}})),
            Some(serde_json::json!({"limits": {"chunk_size": "big"}})),
        ] {
            let plan = super::plan_chunks(limits.as_ref(), 100 * 1024 * 1024)
                .expect("a bad limits payload must not fail the upload");
            assert_eq!(
                plan.chunk_size,
                super::DEFAULT_CHUNK_SIZE,
                "fallback must be the historical 4 MB for {limits:?}"
            );
        }
    }

    /// An unwrapped payload (no `limits` envelope) is read too — the endpoint's
    /// documented shape wraps, but the client layer unwraps envelopes, so both
    /// must work rather than one silently degrading to the fallback.
    #[test]
    fn limits_are_read_wrapped_or_unwrapped() {
        let flat = serde_json::json!({"chunk_size": 104_857_600_u64, "chunks": 500_u64});
        let ten_gb = 10 * 1024 * 1024 * 1024_u64;
        let plan = super::plan_chunks(Some(&flat), ten_gb).expect("ok");
        assert!(ten_gb.div_ceil(plan.chunk_size as u64) <= 500);
    }
