...
//!   `/storage/{node}/read/`, per the published API docs.
...
//!   `9992` is the director's path walk ending unmatched; `1685` is "insufficient signing
...
///
/// `policy_json` is documented as an object in the published API docs; an
/// array / scalar / null is rejected client-side before any state-changing
/// call.
...
/// document object shape in the published API docs.
...
fn parse_fields(items: Vec<Value>) -> Result<Vec<signing::FieldSpec>> {
    items
        ...
        .map(|(i, v)| {
            ...
            // `validation` is documented as an object in the published API
            // docs; a present non-object is a caller error, not something to
            // silently drop — dropping it yields an envelope with no
            // constraints and no warning.
...
/// the required (terminal) stage (per the published API docs; live signed-PDF
...
/// `404`/`1609` instead means a genuine not-found (typo'd id / wrong
/// workspace), so the "poll and retry" framing would be misleading — those
/// callers fall through to the shared generic not-found handling.
...
///   `9992` (the director's path walk ending unmatched, also HTTP 404) is the
///   one exclusion, so it falls through to the route-not-found framing instead
///   of "poll a route no handler ever served". For
...
/// - `9992` (404) → the API director finished the path walk without matching an
///   endpoint. Names the call site only; attributes no side.
...
fn map_signing_error(err: CliError, ctx: &'static str, op: SignOp) -> anyhow::Error {
    ...
    // Code 9992 also surfaces as HTTP 404 and must be EXCLUDED here: it is the
    // API director's path walk ending with no `index.php` matched, so no
    // handler ever ran and a route cannot appear by polling — the poll framing
    // would loop an agent forever. The code-specific match below names that
    // mechanism without diagnosing which side produced it.
    // An unreadable error body is not evidence of anything. When the body
    // could not be obtained (too large / too slow — see
    // [`fastio_cli::error::ERR_BODY_UNAVAILABLE`]) the code is `0` by default,
    // which sails past the `!= 9992` exclusion below and lands a bare 404 in the
    // poll-forever branch. A slow-but-valid `9992` ("route not found") would then
    // tell the user to poll for something that can never appear — the exact
    // outcome the `9992` exclusion exists to prevent.
    ...
    let note = match api.code {
        ...
        // Director path walk ended unmatched (`director.php`). Names the call
        // site; attributes no side — see the constant's docs in `error.rs`.
        9992 => Some(format!(
            "{ctx}: the server's API director finished resolving this request path without \
             matching an endpoint (9992). No handler ran, so polling will not help and this \
             says nothing about the envelope's state. The response cannot say which side is \
             at fault — a path this build calls that the deployment does not expose, and a \
        ...
        // Send precondition not met (checked only at send time).
        201_134 => Some(format!(
...
async fn execute_envelope(command: SignEnvelopeCommands, ctx: &CommandContext<'_>) -> Result<()> {
    match command {
        ...
        SignEnvelopeCommands::Update {
            ...
            // required. Surface a clear, action-specific error before the
            ...
            // `expires_at` / `policy_json` are declarative in the published API
            // docs: omitting one CLEARS it server-side. Warn before sending so
            // a rename-only update doesn't silently drop the expiry/policy.
            if !ctx.output.quiet
        ...
        SignEnvelopeCommands::Void {
            ...
            // has confirmed at the y/N prompt.
...
///
/// `body_json` is documented as an object in the published API docs; a
/// non-object (array / scalar / null) is rejected before any params are built.
/// A supplied `policy_json` must likewise be an object.
...
///
/// `expires_at` and `policy_json` are declarative in the published API contract:
/// the server rewrites them on every update, so omitting one CLEARS it (resets
/// to `null`).
...
/// Stream a signing binary/JSON artifact to `output` via the streaming
...
/// as the published API contract requires.
...
/// not-found (typo'd id / wrong workspace/envelope/document id), not "not
/// ready".
///
...
fn download_ctx(what: &str) -> (&'static str, SignOp) {
    match what {
        ...
        // fetch, so a 404 is a genuine not-found, not "not ready".
...
mod tests {
    ...
    // ─── Empty recipient replace on update rejected pre-network ────────────────
    ...
    // ─── Present-but-mistyped JSON fields are rejected, not dropped ────────────
    ...
    // ─── Non-object body_json / policy_json rejected ───────────────────────────

    #[test]
    fn create_body_json_non_object_rejected() {
        // body_json is documented as an object in the published API docs.
    ...
    fn map_send_precondition_201134_names_the_gap_and_keys_on_code() {
        // A /send precondition failure (201134) gets a hint that names the
    ...
    fn update_declarative_clear_note_fires_only_for_omitted_fields() {
        // expires_at / policy_json are declarative — omitting one clears it.
    ...
    #[test]
    fn an_unreadable_error_body_is_not_treated_as_artifact_not_ready() {
        // Bounding the error-body read introduces a distinct state: "there is
        // a reason and it could not be read." It carries `code: 0` like a
        // genuinely bodyless response, so without the marker check it would
        // pass the `code != 9992` exclusion and a bare 404 would become "poll
        // and retry".
        //
        // The concrete failure: a signing 404 whose valid envelope carries
        // `9992` — route not found, which polling can never resolve — arrives
        // just past the deadline. Losing the code would turn an unrecoverable
        // error into an instruction to poll forever.
        let unreadable = CliError::Api(fastio_cli::error::ApiError::new(
            0,
            Some(fastio_cli::error::ERR_BODY_UNAVAILABLE.to_owned()),
            "API request failed with HTTP 404; the server's error body could not be read"
                .to_owned(),
            404,
        ));
        let mapped = map_signing_error(unreadable, "download", SignOp::SignedFetch);
        let rendered = mapped.to_string();
        assert!(
            !rendered.contains("Poll") && !rendered.contains("poll"),
            "an unreadable body is not evidence the artifact is pending: {rendered}"
        );
        // A genuinely bodyless 404 still maps to not-ready — the exclusion must
        // be keyed on the MARKER, not broadened to every code-0 404.
        let plain = map_signing_error(api_err(0, 404), "download", SignOp::SignedFetch).to_string();
        assert!(
            plain.contains("poll") || plain.contains("Poll"),
            "control: a real bodyless 404 still reads as not-ready: {plain}"
        );
    }

    #[test]
    fn map_artifact_not_ready_wording_is_artifact_appropriate() {
        // The signed-PDF not-ready guidance steers to "completes"; the
        // audit-certificate guidance steers to "terminal state". The signed
        // message must not promise a terminal state (signed PDFs exist only on
        // completion, never on a void), and the audit message must not narrow
        // to "completes" (audit certs exist on any terminal state —
        // completed/declined/voided/expired/failed).
        let signed =
    ...
    fn map_artifact_fetch_not_ready_uses_artifact_variant_and_poll_hint() {
        // A not-ready artifact fetch must re-key onto
    ...
    fn map_artifact_fetch_146422_non_404_status_says_not_ready() {
        // Code 146422 (signed-PDF not-ready) with a non-404 http_status must
        // still map to not-ready, proving the explicit `code == 146422`
    ...
    fn map_general_404_does_not_say_not_ready() {
        ...
        // must not be reframed as "not ready". It defers to the shared 404
        // suggestion (surfaced by the render layer) instead.
        ...
        let hint = mapped
            ...
            .unwrap_or_default();
        // Asserted on the ACTION, not on the words "not found". The shared 404
        // hint is deliberately action-only and cause-free, precisely because
        // 404 is overloaded — a not-yet-ready artifact and a retired route
        // both land here with correct IDs, so the hint may no longer assert
        // that anything is missing. What this test actually protects is
        // that a general 404 falls through to the SHARED hint instead of the
        // "not ready" reframing, and the discovery guidance is that hint's
        // fingerprint.
        assert!(
            hint.to_lowercase().contains("list"),
            "general 404 must resolve to the shared 404 discovery hint: {hint}"
        );
        assert!(
            !hint.to_lowercase().contains("not ready"),
            "general 404 must not resolve to the not-ready hint: {hint}"
        );
        let m = map_signing_error(
            api_err(1609, 404),
    ...
    fn map_artifact_fetch_128301_says_not_ready() {
        // Live audit-not-ready code: 128301 on an artifact fetch must read
    ...
    #[test]
    fn map_artifact_fetch_9992_404_is_route_not_found_not_poll() {
        // 9992 (also HTTP 404) on an artifact fetch must NOT be reframed as
        // "not ready — poll and retry": it is the API director's path walk
        // ending with no `index.php` matched, so no handler ran and a route
        // cannot appear by polling — an agent would loop forever. The copy must
        // name that call site and diagnose NO side.
        for op in [SignOp::SignedFetch, SignOp::AuditFetch] {
            let m =
            ...
            // Wording from the two retired 9992 framings must not reappear.
            for retired in [
                "does not recognize",
                "removed or renamed",
                "cli update",
                ...
                assert!(
                    ...
                    "9992 on {op:?} must not resurrect a retired model, found {retired:?}: {m}"
                );
            }
        }
    }
    ...
    fn map_unknown_route_9992_names_director_and_no_side() {
        ...
        for retired in [
            ...
            assert!(
                !lower.contains(retired),
                "9992 must not resurrect a retired model, found {retired:?}: {m}"
            );
        }
    }
    ...
    fn validate_void_reason_before_prompt() {
        // An empty / oversize reason is rejected by the validator the void
        // flow calls BEFORE confirm_destructive.
