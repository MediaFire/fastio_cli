...
/// Validate the `{email_or_user_id}` path segment of the org add/invite route.
///
/// Per the published API docs the target "is specified as a path parameter: use
/// a **user ID** (19-digit numeric) to add an existing user directly, use an
/// **email address** to send an invitation." Demanding an email here would make
/// adding an EXISTING user unreachable: `fastio member *` is
/// workspace-hardcoded, so the org surface is the only route.
fn validate_email_or_user_id(target: &str) -> Result<()> {
    ...
    anyhow::ensure!(
        ...
        "invalid target '{target}': expected an email address (to invite) or a \
...
pub async fn execute(command: &OrgCommand, ctx: &CommandContext<'_>) -> Result<()> {
    match command {
        ...
        OrgCommand::Workspaces {
            ...
            offset,
            archived,
        } => list_workspaces(ctx, org_id, *limit, *offset, *archived).await,
        OrgCommand::Shares {
            org_id,
            limit,
            offset,
...
mod billing_format {
    ...
    /// or NESTED under a `subscription` object (see the published API docs).
    /// Format wherever
    ...
    ///
    /// The 201 create response carries `setup_intent.client_secret`
    ...
    mod tests {
        ...
        fn format_details_handles_nested_subscription_timestamps() {
            // Timestamps may be nested under a `subscription` object; they must
            // be formatted there too, while top-level handling is preserved.
            let mut v = json!({
                "current_period_end": 1_735_689_600i64,
                "subscription": {
                    "id": "sub_1",
        ...
        fn sanitize_subscribe_removes_client_secret_and_public_key() {
            // The 201 subscribe response carries a real
...
async fn members(cmd: &OrgMembersCommand, ctx: &CommandContext<'_>) -> Result<()> {
    ...
    match cmd {
        ...
        OrgMembersCommand::Invite {
            ...
            validate_org_id(org_id)?;
            validate_email_or_user_id(email)?;
            if let Some(r) = role {
                validate_role(r)?;
            }
...
async fn list_workspaces(
    ...
    validate_org_id(org_id)?;
    let client = ctx.build_client()?;
    let value = api::org::list_workspaces(&client, org_id, limit, offset, archived)
        .await
...
#[cfg(test)]
mod tests {
    /// A 19-digit user ID must be ACCEPTED, not rejected as a bad email.
    ///
    /// Per the published API docs the path target is "a user ID (19-digit
    /// numeric) to add an existing user directly" OR "an email address to
    /// send an invitation". A guard demanding an email would close the only
    /// route: `fastio member *` is workspace-hardcoded, so there would be
    /// no other way to add an existing user to an org.
    #[test]
    fn org_member_target_accepts_a_user_id_or_an_email() {
        use super::validate_email_or_user_id as check;

        check("1234567890123456789").expect("a 19-digit user id is a valid target");
        check("someone@example.com").expect("an email is a valid target");
        check("  1234567890123456789  ").expect("surrounding whitespace is trimmed");

        // Still rejected: neither shape.
        check("").expect_err("empty");
        check("   ").expect_err("whitespace only");
        check("nobody").expect_err("not an email and not an id");
        // Near-misses on the id shape must fall through to the email rule
        // rather than being waved through by a loose numeric check.
        check("123456789012345678").expect_err("18 digits is not a profile id");
        check("12345678901234567890").expect_err("20 digits is not a profile id");
        check("123456789012345678a").expect_err("non-digits are not a profile id");
    }
}
