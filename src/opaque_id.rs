...
//!
//! Source of truth: the opaque-id specification (§5 type map, §4 caveat).
...
/// Strip display hyphens, yielding the canonical (wire) form of an id.
///
/// A formatted Fast.io id inserts a `-` every 5 characters; the canonical form
/// the server stores is unhyphenated. Most endpoints canonicalize whatever they
/// are handed, so either form works and callers need not think about it — but
/// not all of them do, and the canonical form is accepted everywhere the
/// formatted one is. Use this when an endpoint is known to compare an id
/// against a stored value rather than look it up.
#[must_use]
pub fn canonicalize(id: &str) -> String {
    id.trim().chars().filter(|&c| c != '-').collect()
}
...
pub fn classify(id: &str) -> Classification {
    let input = id.trim();
    let raw: String = canonicalize(input);
    let length = raw.len();
...
mod tests {
    ...
    #[test]
    fn canonicalize_strips_hyphens_trims_and_is_idempotent() {
        // Pinned at its own layer: `canonicalize` is public API and was
        // previously exercised only transitively through `classify` and the
        // write-back path builder.
        let formatted = "argw2-322y6-dui3h-4ov3b-zdq7w-nya7";
        let canonical = "argw2322y6dui3h4ov3bzdq7wnya7";
        assert_eq!(canonicalize(formatted), canonical, "display hyphens strip");
        assert_eq!(
            canonicalize(canonical),
            canonical,
            "an already-canonical id must be unchanged (idempotent)",
        );
        assert_eq!(
            canonicalize("  argw2-322y6  "),
            "argw2322y6",
            "outer whitespace is trimmed as well as hyphens stripped",
        );
        assert_eq!(canonicalize(""), "", "empty input yields empty output");
    }
