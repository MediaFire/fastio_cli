...
mod tests {
    ...
    /// The config dir must be 0700 ON DISK — this is a filesystem property, so
    /// asserting it anywhere but on a real `stat` proves nothing.
    ///
    /// Covers BOTH documented behaviours, and the second is the security one:
    /// `ensure_config_dir` re-applies permissions to an EXISTING directory
    /// "to ensure they have not been loosened since creation". A dir that was
    /// chmod'd 0755 by anything else must be tightened back.
    #[cfg(unix)]
    #[test]
    fn ensure_config_dir_is_0700_on_disk_and_retightens_a_loosened_dir() {
        use std::os::unix::fs::PermissionsExt;
        let dir = std::env::temp_dir().join(format!("fastio-cfgperm-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);

        // 1. Freshly created.
        super::ensure_config_dir(&dir).expect("create config dir");
        let mode = std::fs::metadata(&dir).expect("stat").permissions().mode() & 0o777;
        assert_eq!(mode, 0o700, "a new config dir must be 0700, got {mode:o}");

        // 2. Loosened by something else, then re-ensured.
        std::fs::set_permissions(&dir, std::fs::Permissions::from_mode(0o755))
            .expect("loosen to 0755");
        super::ensure_config_dir(&dir).expect("re-ensure existing config dir");
        let mode = std::fs::metadata(&dir).expect("stat").permissions().mode() & 0o777;
        let _ = std::fs::remove_dir_all(&dir);
        assert_eq!(
            mode, 0o700,
            "an EXISTING dir loosened to 0755 must be re-tightened to 0700, got {mode:o}"
        );
    }
}
