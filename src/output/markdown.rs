...
/// The Trojan-Source / terminal-hostile strip set: C0/C1 control characters
/// (except `\n` and `\t`) plus Unicode bidi / zero-width / BOM code points.
///
/// `pub(crate)` because the TABLE and CSV formatters share this exact set (via
/// [`crate::output::strip_terminal_hostile`]): they emit server- and
/// peer-authored strings to a terminal, where an unstripped ESC (0x1B) would
/// let untrusted content — e.g. a room message body — drive the viewer's
/// terminal (title writes, screen manipulation, spoofed output).
pub(crate) fn is_stripped_char(ch: char) -> bool {
    let code = ch as u32;
    if code < 0x20 && ch != '\n' && ch != '\t' {
        return true;
    }
