...
//! key per process (for an individually re-pointed command, whose replacement
//! is not simply its group's new name).
//!
...
///
/// Keyed by the static group name (e.g. `"ripley"`). Wrapped in a [`Mutex`] for
...
/// for whole command groups), this lets a caller supply its own wording — for
/// an individually re-pointed command whose replacement is not simply its
/// group's new name.
///
/// **Currently has no in-tree caller** — its last one, `ripley search`, was
/// removed on 2026-08-29 with the rest of the `/ai/search/` surface. Retained
/// deliberately as public library API: it is the counterpart to
/// [`warn_legacy`], and the next individually re-pointed command needs exactly
/// this. Not dead code to be swept.
///
/// Same guarantees: at most once per `key` per process,
