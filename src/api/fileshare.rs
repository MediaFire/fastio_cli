...
/// `if_version_id` compare-and-swap precondition fails.
...
///
/// Uses [`CliError::Parse`], NOT [`CliError::Config`]: a missing/empty CLI
...
/// interpolated verbatim. PRIVATE by design: exposing it publicly with a
...
/// Parameters for creating a File Share.
///
...
impl CreateFileShareParams {
    ...
    pub fn validate(&self) -> Result<(), CliError> {
        ...
        // An empty title (`title=""`) is the server's title-CLEAR sentinel.
        ...
        // Validate the access tier client-side so library consumers get a
        // clear error (the CLI layer also enforces this via clap).
        validate_access_option(self.access_option.as_deref())?;
        // An empty password is meaningless on create — the contract is a
        ...
        // A directly-supplied `expires_at` that trims to a case-insensitive
...
pub struct UpdateFileShareParams {
    ...
    ///
    /// Title CLEARING (sending an empty title) is deliberately
...
impl UpdateFileShareParams {
    ...
    pub fn validate(&self) -> Result<(), CliError> {
        // Title-clearing is deliberately unsupported (see the field doc), so
        ...
        // Validate the access tier client-side (the CLI layer also enforces
        // this via clap).
        validate_access_option(self.access_option.as_deref())?;
        ...
        // Clearing the password / expiry is ONLY via the explicit
...
/// Parameters for a grant add (POST) or remove (DELETE).
///
...
///
/// `ACCESS_OPTIONS` previously existed only for the command/MCP value parsers;
/// the library validators never checked the supplied value, so a library
/// consumer could POST an unknown tier and only learn of it from a server 400.
/// This protects those consumers (the CLI layer ALSO enforces the set via
/// clap). `None` is valid — the field is optional and the server applies its
/// default. Uses [`CliError::Parse`] (see [`require_id`]); the error lists the
/// accepted values.
...
/// `download::extract_filename` looks for top-level
...
///
/// Routed through [`ApiClient::post_sensitive_form`] (fail-closed on any
...
///
/// A GET that ACTS: it mints a credential. Routed through
...
mod tests {
    ...
    fn empty_id_rejection_is_parse_not_config() {
        // An empty id is a missing-argument problem, NOT a config problem.
    ...
    fn create_validate_rejects_empty_title() {
        // An empty title is the server's title-CLEAR sentinel; clearing is
    ...
    fn create_validate_checks_access_option_membership() {
        // An unknown access tier must be rejected client-side (as Parse).
    ...
    fn create_validate_rejects_empty_password_and_expires_at() {
        // An empty password (e.g. an unset env var) must be rejected rather
    ...
    fn create_validate_rejects_literal_null_expires_at() {
        // A directly-supplied `expires_at` trimming to case-insensitive
    ...
    fn update_validate_rejects_empty_title() {
        // A directly-supplied empty title (`title=""`) is the server's
    ...
    fn update_validate_rejects_directly_supplied_empty_password_and_expires_at() {
        // Clearing is ONLY via the explicit flags. A directly-supplied empty
    ...
    fn grant_validators_return_parse_not_config() {
        // Input-validation failures must be CliError::Parse (no misleading
    ...
    fn parse_conflict_version_extracts_id_else_none() {
        ...
        // THE REAL WIRE SHAPE, captured 2026-08-25 from an actual write-back
        // CAS rejection: raw, 29 chars, lowercase, NO hyphens.
        // The same version was reported by `fileshare versions` as
        // `3f6qo-xul6p-l2swd-uxydx-avqlw-imtj` (34 chars, hyphenated every 5) —
        // one version, two renderings, because only an allowlist of fields gets
        // `format()` applied on the way out. Pin the measured value so nobody
        // later "tightens" this parser against a shape the server never sends.
        assert_eq!(
            parse_conflict_version("CONFLICT_VERSION_MISMATCH:3f6qoxul6pl2swduxydxavqlwimtj"),
            Some("3f6qoxul6pl2swduxydxavqlwimtj")
        );
        // A hyphenated id must ALSO round-trip intact (hyphens are part of the
        // id, not delimiters the parser splits on). NOTE: this fixture came from
        // a documentation example that was later retracted as a hand-written
        // placeholder — it is kept because accepting BOTH renderings is the
        // correct posture, NOT because the server emits this shape here.
        assert_eq!(
    ...
    // ─── Management writes fail closed on a redirect (no body replay) ───
    ...
    ///
    /// Mirrors the `upload.rs` counting-server pattern: it DRAINS the
    ...
    async fn create_fileshare_fails_closed_on_redirect_without_replaying_password() {
        ...
        // The create form can carry a `password` field. A 3xx on the
    ...
    async fn update_fileshare_fails_closed_on_redirect_without_replaying_password() {
        use std::sync::atomic::Ordering;
        // Identical posture for the update form, which can also carry a
        // `password` field.
        let (addr, count) =
