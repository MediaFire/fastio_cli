...
//! endpoints, documented under the `## Unified Search (Grouped by Type)`
//! heading in the published API docs. Unlike the per-type endpoints
//! (`storage/search`, `metadata/search`, comments), this composes a single
//! query into a set of independently paginated, independently health-reported
//! **buckets** (`files`, `metadata`, `comments` — the share endpoint omits
//! `metadata`).
//!
//! This is the **primary** search surface for the CLI and the MCP server; the
//! flat `crate::api::storage::search_files` leg is for a single file list with
//! no metadata or comment hits.
//!
...
/// Maximum length of the unified-search query string, in **Unicode code
/// points** — counted with `chars().count()`, never `len()`. The contract
/// states it as "max 1024 characters; an empty value is rejected" on the
/// `search` parameter.
///
/// Applies to the two unified `/search/` endpoints only. The
/// `/storage/search/` family is deliberately **uncapped** and must stay that
/// way: the CLI has always forwarded longer queries there.
...
impl UnifiedSearchParams {
    ...
    /// Set the search-mode parameters (files bucket only).
    #[must_use]
...
/// non-blank after trimming and at most [`MAX_QUERY_LEN`] characters (see the
/// published API docs). Returns the original (untrimmed) query on success so
...
/// `comments` when commenting is enabled); `metadata` is workspace-only
/// (confirmed against the server, 2026-08-29) and is never
/// requested here.
///
/// Workspace-backed (folder) shares answer HTTP `404` with `error.code`
/// `139420`, which the command layer surfaces as a friendly "not available for
/// folder shares" message. The docs' error table renders that row as
/// `1609 (Not Found)`, but `1609` is the status **class** and never appears in
/// `error.code` — do not match on it.
...
mod tests {
    ...
    #[test]
    fn omitting_search_modes_leaves_the_request_byte_identical() {
        // The server guarantees that omitting every search-mode parameter
        // reproduces today's query and ranking exactly. That guarantee is only
        // worth anything if the client actually sends nothing, so assert the
        // request itself is unchanged rather than trusting the builder.
        let q = UnifiedSearchParams::new()
            .files(Some(1), Some(2))
            .into_query("hello", true);
        for key in ["search_in", "name_match", "case_sensitive"] {
            assert!(!q.contains_key(key), "{key} must be absent when unset");
        }
        assert_eq!(q.len(), 3, "only search + the two bucket params: {q:?}");
    }

    #[test]
    fn search_modes_are_forwarded_when_set() {
        let modes = crate::api::types::SearchModeParams::new()
            .search_in(Some("filename"))
            .name_match(Some("glob"));
        let q = UnifiedSearchParams::new()
            .modes(modes)
            .into_query("*.pdf", false);
        assert_eq!(q.get("search_in").map(String::as_str), Some("filename"));
        assert_eq!(q.get("name_match").map(String::as_str), Some("glob"));
        assert_eq!(q.get("search").map(String::as_str), Some("*.pdf"));
    }

    #[test]
    fn precise_mode_pattern_rules_apply_to_unified_search() {
        let modes = crate::api::types::SearchModeParams::new().name_match(Some("glob"));
        // Blank and >256-char patterns are refused before the request is sent.
        assert!(modes.validate("   ").is_err());
        assert!(modes.validate(&"x".repeat(257)).is_err());
        assert!(modes.validate("*.pdf").is_ok());
    }
