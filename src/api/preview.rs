...
///
/// A GET that ACTS: it preauthorizes access. Routed through
/// [`ApiClient::get_side_effecting`] so a lost response body is never recovered
/// by re-sending — that would preauthorize a second time.
...
fn validate_transform_params(params: &TransformUrlParams<'_>) -> Result<(), CliError> {
    ...
    // The backend requires every integer dimension to be strictly > 0, so a
    // supplied 0 is rejected server-side — reject it client-side with a clear
    // message instead of a 400. Rotate is excluded: it is a fixed choice for
    // which 0 is a valid value.
    for (label, value) in [
        ("--width", params.width),
...
pub async fn get_transform_url(
    ...
    // transform name (it ignores width/height/output-format/etc.). It is a GET
    // that ACTS — it mints — so it goes out on the non-replaying path: a lost
    // response body must never be recovered by re-sending, which would mint a
    // second token.
    ...
    let response: Value = client.get_side_effecting(&request_path).await?;
    let token = response
        .get("token")
