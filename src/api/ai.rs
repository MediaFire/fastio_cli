...
/// Supports chat creation, message send/read, and summarize. Search is NOT an
/// AI endpoint — see [`crate::api::search`] for the unified `/search/` family.
...
///
/// Mirrors the `/ai/agent/` create contract in the published API docs.
/// `privacy`/`kind` are workspace-only — share chats reject them; the caller
/// is responsible for not populating them in a share context.
#[derive(Debug, Default, Clone)]
#[non_exhaustive]
pub struct ChatCreateOptions {
    ...
    pub scope: ChatScope,
    /// Replay guard for the FIRST turn of the chat.
    ///
    /// `None` makes one up; see [`new_idempotency_key`] for why the client
    /// generates it rather than letting the server.
    pub idempotency_key: Option<String>,
}

/// Mint a per-turn idempotency key.
///
/// An idempotency key is always sent: without one, the retry advice on a
/// `SEQUENCE_FAILURE` — "retry the SAME idempotency key" — cannot be followed,
/// because there never was one, and the retry creates a **second billable
/// turn** instead of returning the first.
///
/// The published API docs allow the key to be omitted "to have one generated",
/// but a server-generated key is useless as a replay guard: the client cannot
/// re-send a key it never saw. Generating here is what makes the retry actually
/// replay.
///
/// 16 random bytes, base64url — 22 characters, well inside the documented
/// 64-character ceiling. Uses the same `getrandom` + base64url pairing as the
/// PKCE verifier in `auth::pkce`.
#[must_use]
pub fn new_idempotency_key() -> String {
    ...
    if getrandom_crate::getrandom(&mut buf).is_err() {
        ...
        // but no worse than not sending the field at all.
        return String::new();
    }
...
/// `/ai/agent/` contract in the published API docs. File/folder context is
/// carried by the single structured `references` form field (see
...
pub async fn create_chat(
    ...
    insert_idempotency_key(&mut form, options.idempotency_key.as_deref());
...
/// **form-encoded** per the `/ai/agent/` contract in the published API docs.
/// The chat type is inherited from the chat, so `type` is not resent;
/// `personality` is dead on the migrated endpoint and also not sent.
/// File/folder context uses the single structured `references` field (see
/// [`ChatScope`] / [`build_references`]).
pub async fn send_message(
    ...
    insert_idempotency_key(&mut form, idempotency_key);
    let path = format!(
...
/// Insert `idempotency_key`, generating one when the caller has no preference.
///
/// An EMPTY explicit key is treated as "no key" rather than sent as a blank
/// field — the server would take a blank string as a real key, and every blank
/// request would then collide with every other one.
fn insert_idempotency_key(form: &mut HashMap<String, String>, key: Option<&str>) {
    let key = match key {
        Some(k) if !k.trim().is_empty() => k.trim().to_owned(),
        Some(_) | None => new_idempotency_key(),
    };
    if !key.is_empty() {
        form.insert("idempotency_key".to_owned(), key);
    }
}
...
/// `/ai/agent/` contract in the published API docs the path carries a
/// **trailing slash**. Both segment values are URL-encoded; higher-level
...
pub async fn cancel_message(
    ...
    // The cancel endpoint's body is documented as **Empty**, so send a
    // body-less POST rather than a JSON `{}` — `post_empty_raw` sets
...
///
/// `GET /workspace/{workspace_id}/ai/agent/{chat_id}/messages/list/[{offset}]`
///
/// The offset is a **path segment**, not a query parameter, per the published
/// API docs: "A numeric path segment after `/messages/list/` is a pagination
/// offset (e.g. `.../messages/list/50`)." Sent as `?offset=` it is swallowed —
/// this endpoint does not declare that query parameter — and `--offset 50`
/// silently returns page 1, so paging through a long chat re-reads the first
/// page forever.
///
/// The endpoint declares **no query parameters at all** and "returns all
/// messages in chronological order", so there is no server-side `limit`
/// either; a `?limit=` is likewise swallowed. Capping the list is done by the
/// caller on the returned items.
pub async fn list_messages(
    ...
    client
        .get(&messages_list_path(workspace_id, chat_id, offset))
        .await
}

/// Build the list-messages path, appending the offset as a trailing segment.
///
/// Extracted and tested because the difference between a working offset and a
/// silently-ignored one is invisible in the response: both return page 1.
fn messages_list_path(workspace_id: &str, chat_id: &str, offset: Option<u32>) -> String {
    let base = format!(
    ...
    match offset {
        // Offset 0 is page 1, which the bare path already returns — appending it
        // would send a segment for no reason.
        None | Some(0) => base,
...
///
/// Mirrors the `/ai/agent/list/` query surface in the published API docs:
/// - `kind` — `user` (default), `agent`, or `all`.
...
/// `.../ai/agent/list/deleted` when `options.deleted` is set). The `kind`
/// filter (`user`/`agent`/`all`) is threaded through as a query param.
...
///
/// Returns up to 40 most-recent transactions. **Workspace-only** — the
/// published API docs list no share equivalent; the caller is responsible for
/// rejecting a share context before calling this.
...
///
/// **Share-only** per the published API docs; the caller must reject a
/// workspace context before calling this. The optional `user_context` form
/// field guides generation. Generated values are applied directly to the
/// share server-side.
...
/// in the body is ignored server-side.
...
// There is intentionally NO search builder in this module. Search is not an AI
// endpoint: the unified grouped-bucket search (`api::search`, `/search/`) is
// the primary surface, and `api::storage::search_files` is the flat
// filename/content leg. Do not add a third one here.
...
/// CSV — see the published API docs). The API generates AI-powered markdown
/// with temporary download URLs that can be pasted into an external chatbot.
...
///
/// This builder does NOT enforce the API's documented 1-25 `files` bound —
/// callers are responsible for the client-side length check before calling
/// this; both the CLI `summary` handler and the MCP `share-generate` handler
/// reject `> 25` (and empty) before the network round-trip.
#[must_use]
...
/// `application/x-www-form-urlencoded` bodies per the published API docs.
#[allow(clippy::implicit_hasher)]
...
/// while a SHARE ask returns `{turn: {state, …}}`; the published API docs
/// state: "The message detail is returned under a `message` object (a `turn`
/// object on the share endpoint)." When neither wrapper is present (an
/// already-unwrapped or bare detail) the top-level body is returned unchanged.
/// Centralizing the wrapper rule here keeps the CLI `ask`/render and MCP `ask`
/// paths from each re-deriving — and drifting on — it; without it a share
/// `needs_input` turn's `state` is never read (no `message` key), so the wait
/// loop polls to a misleading timeout and the clarification is missed.
#[must_use]
...
/// Is an agent TURN `status` terminal (the poll/wait loop should stop)?
///
/// **Turn statuses only.** The name says `turn` deliberately: `errored` below is
/// tolerated here, and `errored` is a metadata-job status that the published API
/// docs describe as *sometimes non-terminal* — a retryable extract-all failure
/// publishes `errored` with a `completed_at` and then resumes under the same
/// `job_id`. This predicate is safe only because its callers are the two AI wait
/// loops; pointing it at a job poll would be wrong.
///
/// The documented set is `pending` / `running` (non-terminal) and `complete`,
/// `failed`, `cancelled`, `lost`, `needs_input` (terminal). Plus `errored`,
/// which is NOT a documented turn status; see the match body.
///
/// **Terminal does not mean successful.** Four of the six terminal values
/// carry no answer. A caller that maps every terminal status onto a
/// "here is your answer" rendering will present `{"response": ""}` as a finished
/// reply. Pair this with [`is_failed_state`].
#[must_use]
pub fn is_terminal_turn_status(state: &str) -> bool {
    matches!(
        state,
        // The documented set: `pending`/`running` are non-terminal, the rest
        // are terminal.
        "complete" | "failed" | "cancelled" | "lost" | "needs_input"
        // NOT a documented turn status — `errored` belongs to metadata jobs.
        // Retained deliberately: treating it as terminal can only end a poll
        // early for a value the server does not send, whereas dropping it
        // could poll a real one to a timeout if some deployment still emits
        // it.
        | "errored"
    )
}

/// Is a terminal turn status one that carries **no answer**?
///
/// `failed`, `cancelled` and `lost` are documented terminal statuses that end
/// a turn without producing a reply; `errored` is included for the same reason
/// [`is_terminal_turn_status`] tolerates it.
/// `needs_input` is deliberately **excluded** — it is a non-answer too, but it
/// carries a clarifying question and already has its own rendering.
///
/// Exists because "terminal" and "successful" are not the same question, and
/// conflating them is how a cancelled turn renders as an empty success.
#[must_use]
pub fn is_failed_state(state: &str) -> bool {
    matches!(state, "failed" | "cancelled" | "lost" | "errored")
}

/// Read a turn's processing status from a message-detail object.
///
/// **The field is `status`, not `state`.** The published API docs are explicit:
/// "There is no `state`, `personality`, `response`, `author_name`, or top-level
/// `text`/`citations`/`events` field … the processing state is `status`."
///
/// A client that reads `state` instead sees `""` on every poll, so
/// [`is_terminal_turn_status`] never matches and **every `ripley ask` runs its
/// full wait budget and reports a timeout even for an answer that finished in
/// seconds**.
///
/// `state` is still accepted as a legacy fallback — it costs nothing and keeps
/// an older deployment working — but `status` wins when both are present.
///
/// Accepts a **string or a numeric** JSON value, because the platform returns
/// some scalars either way. Both the CLI wait loop and the MCP one call this, so
/// the two stay in lockstep by construction rather than by two helpers that have
/// to be kept matching.
#[must_use]
pub fn turn_status(msg: &Value) -> String {
    ...
    msg.get("status")
        .and_then(scalar)
        .or_else(|| msg.get("state").and_then(scalar))
        .unwrap_or_default()
}

/// Extract the answer text from a completed turn.
///
/// The answer lives at **`result.answer`** — the published API docs describe
/// `result` as the "Decompressed answer blob: `answer`, `references`,
/// `citations`, `truncated`". The `text`, `response.text` and bare `text`
/// locations probed by older clients do not exist on this contract, so a wait
/// loop that reads only those renders an empty answer.
///
/// The legacy locations are retained as fallbacks after `result.answer`.
#[must_use]
pub fn extract_answer_text(msg_data: &Value, msg: &Value) -> String {
    msg.get("result")
        .and_then(|r| r.get("answer"))
        .and_then(Value::as_str)
        .or_else(|| msg_data.get("text").and_then(Value::as_str))
        .or_else(|| {
            msg.get("response")
...
/// Whether the server marked the answer as truncated (`result.truncated`).
///
/// The published API docs say to surface this so users know the answer is
/// incomplete.
#[must_use]
pub fn answer_was_truncated(msg: &Value) -> bool {
    msg.get("result")
...
/// body, if present.
///
/// A `needs_input` turn carries a single clarifying question instead of a full
/// answer. The question lives in a `clarification` object
/// (`{type, question}`), present at one of several locations checked in
/// priority order:
...
///    (the Ripley SSE contract §2 — distinct from the
...
pub fn extract_clarification_question(msg_data: &Value) -> Option<String> {
    ...
    // The documented location is probed first: with the legacy candidates
    // ahead of it, a payload carrying both a stale top-level `clarification`
    // and the current `result.clarification` returns the stale one. Correct
    // precedence is documented-then-legacy, not the order the probes were
    // historically added in.
    msg.get("result")
        .and_then(clarification_q)
        .or_else(|| msg_data.get("result").and_then(clarification_q))
        .or_else(|| clarification_q(msg_data))
...
mod tests {
    ...
    /// The key must be unique, non-empty, and inside the documented ceiling.
    ///
    /// The published API docs cap it at 64 characters. A generator that
    /// repeated itself would be worse than none: every turn would collide with
    /// the previous one and the server would replay an unrelated answer.
    #[test]
    fn idempotency_keys_are_unique_and_within_the_documented_limit() {
        let keys: std::collections::HashSet<String> =
            (0..64).map(|_| super::new_idempotency_key()).collect();
        assert_eq!(keys.len(), 64, "keys must not repeat");
        for k in &keys {
            assert!(!k.is_empty(), "a key must not be empty");
            assert!(k.len() <= 64, "key exceeds the 64-char ceiling: {k}");
            assert!(
                k.chars()
    ...
    /// An explicitly-supplied key wins; a blank one does NOT become a blank
    /// field, because the server would take "" as a real key and every blank
    /// request would then replay the same turn.
    #[test]
    fn supplied_idempotency_key_wins_and_blank_is_not_sent_as_blank() {
        let mut form = std::collections::HashMap::new();
        super::insert_idempotency_key(&mut form, Some("  reuse-me  "));
        assert_eq!(
        ...
        super::insert_idempotency_key(&mut blank, Some("   "));
        let generated = blank.get("idempotency_key").expect("a key is still sent");
        assert!(
            !generated.trim().is_empty(),
            "a blank key must be replaced, not sent"
        );
    }

    /// The offset is a path segment, not `?offset=`.
    ///
    /// Both renderings return HTTP 200 and both return page 1 when the offset is
    /// ignored, so the response cannot tell you which one you sent. Only the
    /// URL can, which is why this asserts the full literal.
    #[test]
    fn messages_list_offset_is_a_path_segment() {
        assert_eq!(
            super::messages_list_path("123", "chat1", Some(50)),
            "/workspace/123/ai/agent/chat1/messages/list/50"
        );
        // No offset, and offset 0, are both "page 1" — the bare path.
        assert_eq!(
            super::messages_list_path("123", "chat1", None),
            "/workspace/123/ai/agent/chat1/messages/list/"
        );
        assert_eq!(
            super::messages_list_path("123", "chat1", Some(0)),
            "/workspace/123/ai/agent/chat1/messages/list/"
        );
        // A query-string offset is exactly the defect being pinned out.
        assert!(
            !super::messages_list_path("123", "chat1", Some(50)).contains("offset="),
            "the offset must not be sent as a query parameter"
        );
        let p = super::messages_list_path("a/b", "c d", Some(7));
        assert!(p.contains("a%2Fb") && p.contains("c%20d"), "{p}");
    }

    use super::{
        CONVERSATION_TOO_LARGE_CODES, ChatCreateOptions, ChatScope, agent_chat_path,
        answer_was_truncated, build_cancel_path, build_references, build_share_form,
        build_subjects, extract_answer_text, extract_clarification_question,
        is_terminal_turn_status, message_detail, parse_cancel_response, turn_status,
    };
    use crate::error::CliError;
    use serde_json::{Value, json};

    #[test]
    fn is_terminal_turn_status_includes_needs_input() {
        // The documented terminal set. `failed`/`cancelled`/`lost` must be
        // included: a cancelled turn — which is exactly what `ripley cancel`
        // produces — otherwise polls to a misleading timeout. `errored` is not
        // a documented turn status but is tolerated.
        for s in [
            "complete",
            "failed",
            "cancelled",
            ...
            assert!(is_terminal_turn_status(s), "{s} must be terminal");
        }
        // `pending`/`running` are the documented NON-terminal pair.
        for s in [
            "pending",
            "running",
            "ready",
            ...
            assert!(!is_terminal_turn_status(s), "{s} must NOT be terminal");
        }
    }

    /// The regression this test guards.
    ///
    /// A realistic completed turn as the platform actually sends it: `status`,
    /// no `state`, the answer under `result.answer`, no top-level `text`. A
    /// client that reads `state` (absent ⇒ `""` ⇒ never terminal) and probes
    /// `text`/`response.text` for the answer (also absent) **runs its full
    /// ~300s budget and then reports a timeout for an answer that was already
    /// sitting there**.
    ///
    /// This body contains none of the fields those legacy locations use, which
    /// is the point — it fails against that shape and passes against this one.
    #[test]
    fn a_completed_turn_as_the_platform_sends_it_is_terminal_and_yields_its_answer() {
        let body = json!({
            "message": {
                "id": "am_aBcDeFgH",
                "status": "complete",
                "result": {
                    "answer": "The contract terminates on 30 days' notice.",
                    "citations": [{"node_id": "n1"}],
                    "truncated": false
                }
            }
        });
        let msg = message_detail(&body);

        let status = turn_status(msg);
        assert_eq!(status, "complete", "the field is `status`, not `state`");
        assert!(
            is_terminal_turn_status(&status),
            "a completed turn must end the wait loop"
        );
        assert_eq!(
            extract_answer_text(&body, msg),
            "The contract terminates on 30 days' notice.",
            "the answer lives at result.answer"
        );
        assert!(!answer_was_truncated(msg));
    }

    /// `status` wins over a legacy `state`, and `state` alone still works so an
    /// older deployment is not broken by the fix.
    #[test]
    fn turn_status_prefers_status_and_falls_back_to_state() {
        assert_eq!(turn_status(&json!({"status": "complete"})), "complete");
        assert_eq!(turn_status(&json!({"state": "complete"})), "complete");
        assert_eq!(
            turn_status(&json!({"status": "running", "state": "complete"})),
            "running",
            "`status` is the real field and must win when both are present"
        );
        assert_eq!(turn_status(&json!({})), "");
    }

    /// `result.truncated` means the answer is incomplete and the published API
    /// docs say to surface it; without it a clipped answer is presented as a
    /// whole one.
    #[test]
    fn a_truncated_answer_is_detected() {
        let msg = json!({"result": {"answer": "partial…", "truncated": true}});
        assert!(answer_was_truncated(&msg));
        assert!(!answer_was_truncated(&json!({"result": {"answer": "x"}})));
        assert!(!answer_was_truncated(&json!({})));
    }

    /// The old locations still resolve, after the real one — a deployment that
    /// has not migrated keeps rendering rather than going blank.
    #[test]
    fn legacy_answer_locations_still_resolve_behind_the_real_one() {
        let msg = json!({"response": {"text": "legacy"}});
        assert_eq!(extract_answer_text(&json!({}), &msg), "legacy");
        // But `result.answer` outranks them all.
        let both = json!({"result": {"answer": "current"}, "response": {"text": "legacy"}});
        assert_eq!(extract_answer_text(&json!({}), &both), "current");
    }
    ...
    fn share_turn_needs_input_is_detected_terminal_via_helpers() {
        ...
        // misleading timeout.
        let body = json!({"turn": {"status": "needs_input"}});
        let detail = message_detail(&body);
        // Use the SAME helper the wait loops use: reading `state` off the
        // detail by hand would exercise a path production does not take.
        let state = turn_status(detail);
        assert_eq!(state, "needs_input");
        assert!(
            is_terminal_turn_status(&state),
            "a share needs_input turn must terminate the wait loop"
        );
    }

    /// The contract puts the clarification on the DETAIL's `result`
    /// (`message.result.clarification` / `turn.result.clarification`), not only
    /// on the top-level `result`. A probe that checks the top level alone
    /// discards a real clarifying question and reports that none was included.
    #[test]
    fn clarification_is_found_at_the_documented_detail_result_location() {
        let ws = json!({
            "message": {"status": "needs_input",
                        "result": {"clarification": {"type": "q", "question": "Which quarter?"}}}
        });
        assert_eq!(
            extract_clarification_question(&ws).as_deref(),
            Some("Which quarter?"),
            "workspace: message.result.clarification.question"
        );
        let share = json!({
            "turn": {"status": "needs_input",
                     "result": {"clarification": {"question": "Which contract?"}}}
        });
        assert_eq!(
            extract_clarification_question(&share).as_deref(),
            Some("Which contract?"),
            "share: turn.result.clarification.question"
        );
    }

    #[test]
    fn clarification_from_share_turn_wrapper() {
        // SHARE asks return the detail under a `turn` object; the clarification
        // nested there must be found just like the `message` case.
        let body = json!({
            "turn": {
                "state": "needs_input",
    ...
    fn clarification_from_service_level_result_object() {
        ...
        // (the Ripley SSE contract §2), distinct from the `{result:"yes"|"no"}`
    ...
    fn conversation_too_large_codes_are_the_four_call_sites() {
        ...
        // MCP send-error mappers (the Ripley SSE contract §4).
        assert_eq!(
    ...
    fn cancel_path_workspace_scope_uses_agent_with_trailing_slash() {
        ...
        // cancel per the published API docs.
