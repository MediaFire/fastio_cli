...
pub struct UpdateOrgParams<'a> {
    ...
    /// New org domain slug — the `acme` in `acme.fast.io`. Globally unique,
    /// lowercase alphanumerics and hyphens. NOT a bring-your-own domain (that
    /// feature is retired); this is the org's URL slug.
    ///
    /// Deliberately states no length bound: the CLI does not enforce one, and
    /// two platform-side sources disagree (2-63 vs 2-80). Stating an
    /// unenforceable number the server owns is how the help text goes stale
    /// silently — the server is the authority.
...
///
/// `archived` defaults to `"false"` server-side, so an archived workspace is
/// invisible here unless it is asked for explicitly.
...
#[cfg(test)]
mod tests {
    use super::CreateWorkspaceParams;
    use super::{
    ...
    fn create_workspace_omits_intelligence_when_unset() {
        ...
        for k in ["folder_name", "name", "perm_join", "perm_member_manage"] {
            assert!(form.contains_key(k), "`{k}` is required and must be sent");
        }
    }
