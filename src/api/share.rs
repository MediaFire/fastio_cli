...
///
/// `confirm` must be the share's `custom_name` or numeric `id`, and is
/// delivered as a **query parameter**: the server does not read a request body
/// on `DELETE` here. Sending it as a form rejects the delete with `130987`
/// ("The `confirm` field is required. Pass the share's `custom_name` or numeric
/// `id` as the `confirm` query parameter") and the share survives.
