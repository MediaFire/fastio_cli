...
//!
//! Source-of-truth note: the published API docs' endpoint-summary table is
//! STALE (its unsuffixed routes 404 live); the suffixed matrix above is
//! live-verified. Those docs remain authoritative for request/response body
//! shapes, lifecycle semantics, field meanings, and identifier formats.
//!
...
//!
//! Per the signing contract, document and
...
//! [`ApiClient::download_file_stream`] (the streaming helper:
...
/// Hard cap on documents per envelope, per the published API docs. Exceeding it
...
/// Maximum length of a void/decline reason, in **BYTES** — and bytes is the
/// server's own unit, not a client convenience: the published API docs state
/// in two places that the `reason` field is "capped at 1024 bytes".
///
/// **Do not change this to code points.** It is the one text-field limit in
/// this crate the server counts in bytes, so it looks like the odd one out next
/// to the code-point name/query caps (`MAX_PATTERN_LEN`, `MAX_QUERY_LEN`,
/// `SEGMENT_MAX_CHARS`, `validate_filename`'s 255) — but converting it would
/// wrongly admit a ~1024-character non-ASCII reason of ~3 KB that the server
/// rejects. Counted with `len()` deliberately.
...
/// One source document placement in a create/update request, per the published
/// API docs.
///
...
pub struct DocumentSpec {
    /// Existing document id to KEEP on a declarative update.
    pub id: Option<String>,
    /// Source storage node id to copy into the envelope (create / update-add).
    pub source_node_id: Option<String>,
    /// Pinned source version id.
...
/// One recipient in a create/update request (published API docs).
#[derive(Debug, Clone, Default)]
#[non_exhaustive]
pub struct RecipientSpec {
    ...
    /// E.164 phone (REQUIRED when `auth_method=sms_otp`).
...
/// One field placement in a create/update request, per the published API docs.
///
/// `recipient_email` and `document_index` cross-reference the recipients /
/// documents lists by value and by 0-based position respectively. Coordinates
...
pub struct FieldSpec {
    ...
    /// Optional pre-fill value as a JSON string.
    pub value_json: Option<String>,
    /// Optional `validation` OBJECT constraining what a signer may submit.
    ///
    /// This must be forwarded verbatim: `--body-json` routes through the same
    /// parser, so if it is not threaded through, an envelope built from the
    /// documented example in the published API docs is created with no field
    /// constraints and no warning that the rules were discarded.
    ///
    /// Sent as a JSON **object** under the key `validation`, unlike `value_json`
    /// which is a JSON *string*. The server echoes it back as `validation_json`
    /// on the field resource — a different key, on the response side only.
    /// Contents are the server's to police: an unsupported key, or a
    /// `validation` on a signature/initial/checkbox field, is rejected with
    /// `1605`.
...
impl FieldSpec {
    ...
    /// Set the `validation` constraint object.
    #[must_use]
    pub fn validation(mut self, value: Option<Value>) -> Self {
    ...
    fn to_json(&self) -> Value {
        ...
        // `value_json` is documented as a JSON STRING; the caller passes the
        // already-encoded string and the server stores it verbatim, so it is
        // sent as a JSON string value (not re-parsed here).
        if let Some(v) = &self.value_json {
            obj.insert("value_json".to_owned(), Value::String(v.clone()));
        }
        // An OBJECT, not a string — the opposite of `value_json` above, and the
        // request key (`validation`) differs from the response key
        // (`validation_json`).
        if let Some(v) = &self.validation {
            obj.insert("validation".to_owned(), v.clone());
        }
        Value::Object(obj)
    }
}
...
/// Parameters for creating a draft envelope, per the published API docs.
///
...
impl CreateEnvelopeParams {
    ...
    /// Validate the client-side caps from the published API docs BEFORE any
    ...
    /// Serialize to the exact documented create-request JSON body.
    #[must_use]
    fn to_json(&self) -> Value {
        let mut obj = serde_json::Map::new();
...
/// Parameters for updating a draft envelope, per the published API docs.
///
...
/// the published docs require recipients on update). `fields` is a full
/// replacement when supplied (omit to keep, `[]` to clear). `documents` is
/// optional — omit (`None`) to leave the set unchanged, or supply it for a full
/// declarative replacement (1..=20 must remain).
///
...
/// `null`) — re-send the current value to retain it. This is the OPPOSITE of
/// `name` / `documents` / `fields`, which are preserved when omitted; the
/// command layer warns when an update would clear them.
///
...
impl UpdateEnvelopeParams {
    ...
    ///   replacement per the published API docs; the live server 400s an empty
    ///   update, so an absent OR empty recipients list is rejected before the
    ///   network.
    /// - When `documents` is supplied it must hold 1..=20 entries; a `None`
    ///   document set leaves it unchanged.
    ///
    /// # Errors
    ///
    ...
    /// Serialize to the exact documented update-request JSON body.
    #[must_use]
    fn to_json(&self) -> Value {
        let mut obj = serde_json::Map::new();
        if let Some(name) = &self.name {
        ...
        // clears the field server-side (resets to `null`). We emit them only
        // when supplied — omission IS the caller's intent to clear (do not
        // "helpfully" always-insert them).
        if let Some(exp) = &self.expires_at {
            obj.insert("expires_at".to_owned(), Value::String(exp.clone()));
        }
        if let Some(policy) = &self.policy_json {
...
/// Filters for the envelope-list endpoint, per the published API docs.
///
...
///
/// `POST /workspace/{id}/sign_envelopes/create/` (JSON body; see the published
/// API docs). The caller-side caps are validated first via
...
///
/// `POST /workspace/{id}/sign_envelopes/{envelope_id}/update/` (JSON body). The
/// endpoint accepts both POST and PATCH; the CLI uses POST as the portable
/// default (some CDNs/proxies block PATCH).
///
/// Per the published API docs, `expires_at` and `policy_json` are
/// **declarative**: an update that omits them CLEARS them server-side (`name` /
/// `documents` / `fields` are preserved when omitted). The command layer warns
/// before sending an update that would clear them.
...
/// Send a draft envelope (draft → sent; idempotent retry).
///
...
pub async fn send_envelope(
    ...
    // `/send/` is a bodyless POST per the published API contract: send no body
    // and no `Content-Type`, matching the documented shape exactly rather than
    // posting `{}` with a JSON content-type. The send response is a named-key
    // boolean envelope (`{"result": true, …}` — the post-send envelope shape,
    // NOT a standard `response`-wrapped payload), which `post_empty` preserves
    // verbatim since there is no `response` key to unwrap.
    client.post_empty(&path).await
}

/// Validate a void/decline `reason` against the documented contract caps: it
/// must be non-blank and at most [`MAX_REASON_BYTES`].
///
...
/// Void a non-terminal envelope (terminal → `1660`).
///
...
/// Manually re-drive a stuck envelope through the self-healing recovery
/// routine.
///
...
/// Mint the calling workspace member's signing link for a sign envelope, per
...
/// document's `source_node_id` through `/storage/{node}/read/`.
...
/// envelope completes; `403` until the envelope is fully completed.
...
mod tests {
    ...
    /// A field's `validation` object must reach the wire.
    ///
    /// The CLI and MCP parsers share this code path — `--body-json` included —
    /// so if `validation` is not carried through, a caller copying the
    /// documented example from the published API docs gets an envelope whose
    /// fields enforce nothing, with no indication the rules were discarded. On
    /// a signing surface that is a correctness problem, not a cosmetic one.
    #[test]
    fn field_validation_object_is_sent_as_an_object() {
        let spec = super::FieldSpec::new()
            .field_type(Some("text".to_owned()))
            .validation(Some(serde_json::json!({
                "min_length": 2, "max_length": 64, "pattern": "[A-Za-z .'-]+"
            })));
        let wire = spec.to_json();

        let v = wire.get("validation").expect("`validation` must be sent");
        assert!(
            v.is_object(),
            "it is an OBJECT per the published API docs, unlike `value_json` \
             which is a string — sending it stringified would be rejected: {v}"
        );
        assert_eq!(v.get("min_length"), Some(&serde_json::json!(2)));
        assert_eq!(v.get("pattern"), Some(&serde_json::json!("[A-Za-z .'-]+")));

        // `validation_json` is the RESPONSE key the server echoes back; the
        // request key is `validation`. Sending the response spelling would be
        // dropped as undeclared.
        assert!(
            wire.get("validation_json").is_none(),
            "must not send the response-side spelling: {wire}"
        );
    }

    /// Absent stays absent — the field is optional and must not appear as null.
    #[test]
    fn field_without_validation_omits_the_key() {
        let wire = super::FieldSpec::new()
            .field_type(Some("signature".to_owned()))
            .to_json();
        assert!(
            wire.get("validation").is_none(),
            "an unset validation must be omitted, not sent as null: {wire}"
        );
    }
    ...
    fn list_params_to_query_maps_status_to_envelope_status() {
        let q = ListEnvelopesParams::new()
            ...
            .offset(Some(10))
            .to_query();
        // `--status` → query key `envelope_status`, CSV passthrough.
        assert_eq!(
    ...
    // ─── Param → JSON shape (documented create body) ──────────────────────────
    ...
    fn create_params_serialize_to_documented_shape() {
        ...
        // Top-level keys per the published API docs.
    ...
    fn update_omits_documents_when_none_but_includes_recipients_replace() {
        ...
        let body = params.to_json();
        // documents absent → set unchanged.
    ...
    fn update_validate_requires_recipients() {
        // Recipients are REQUIRED on update (full replacement, >= 1). An
    ...
    fn extract_sign_envelope_named_key_and_response_wrapper() {
        // Named-key shape (published API docs) — what the client returns when
    ...
    fn extract_sign_envelopes_prefers_envelopes_key() {
        // Live list shape keys on `envelopes`.
