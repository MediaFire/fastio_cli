...
//!
//! Maps to the owner/admin REST surface documented at
//! `~/vividengine/llms/signing.txt`. A SignEnvelope is a Profile holding up to
//! twenty source PDFs sent to one or more recipients for electronic signature;
//! envelopes are parented to a **Workspace** OR an **Organization** and flow
//! through a small lifecycle state machine (draft → sent → in_progress →
...
//! **`application/json`** request bodies (`signing.txt:291`, `:348`, `:379`).
//! CRUD/lifecycle therefore route through [`ApiClient::post_json`] /
//! [`ApiClient::patch_json`] (NOT the form helpers); the soft-delete routes
//! through [`ApiClient::delete`].
//!
//! ## Response envelope shape
//!
//! Signing responses use a **named-key** envelope, e.g.
//! `{"result": true, "sign_envelope": {…}}` for a single envelope
//! (`signing.txt:176-282`) and `{"result": true, "sign_envelopes": […]}` for a
//! list — NOT the standard `{"result": …, "response": {…}}` wrapper. The
//! client's [`ApiClient::handle_response`] only unwraps a `response` key, so
//! for these endpoints it returns the **whole envelope** (minus
//! `current_api_version`). That is the correct render target — the markdown
//! renderer needs the top-level `result` for its preamble and then emits each
//! named key (`sign_envelope` / `sign_envelopes`) as a section. The
//! [`extract_sign_envelope`] / [`extract_sign_envelopes`] helpers are provided
//! for callers/tests that need the bare object/array.
//!
//! ## Binary downloads (stream, do NOT round-trip through storage)
//!
//! Per `signing.txt:155` + the plan's Cross-Model Addendum, document and audit
//! byte endpoints **stream the bytes directly under Bearer auth** — a
...
//!
//! ## Identifier formats (`signing.txt:608-618`)
//!
//! Three id kinds, all treated as opaque `String` and URL-encoded into the
//! path: the **envelope id** / **workspace id** / **org id** are 19-digit
//! numeric profile-id strings; the **document id** / **recipient id** /
//! **field id** are hyphenated base32 `OpaqueId`s. Never parse or assume
//! structure.
...
/// Build the `/{parent}/{id}/sign_envelopes/` base path for a workspace- or
/// org-parented envelope collection.
///
/// `parent_type` MUST be exactly `"workspace"` or `"org"` (`signing.txt:27-54`,
/// `:625`); any other value is rejected with a clear [`CliError::Config`]
/// before any network call. The `parent_id` is URL-encoded.
///
/// # Errors
///
/// Returns [`CliError::Config`] when `parent_type` is not `workspace` or `org`.
pub fn parent_path(parent_type: &str, parent_id: &str) -> Result<String, CliError> {
    match parent_type {
        "workspace" | "org" => Ok(format!(
            "/{parent_type}/{}/sign_envelopes/",
            urlencoding::encode(parent_id)
        )),
        other => Err(CliError::Config(format!(
            "invalid sign-envelope parent type '{other}': must be 'workspace' or 'org'"
        ))),
    }
}

/// Build the path to a single envelope:
/// `/{parent}/{id}/sign_envelopes/{envelope_id}/`.
fn envelope_path(
    parent_type: &str,
    parent_id: &str,
    envelope_id: &str,
) -> Result<String, CliError> {
    let base = parent_path(parent_type, parent_id)?;
    Ok(format!("{base}{}/", urlencoding::encode(envelope_id)))
}

/// Build the path to an envelope sub-action (`send` / `void`):
/// `/{parent}/{id}/sign_envelopes/{envelope_id}/{action}/`.
fn envelope_action_path(
    ...
    let base = envelope_path(parent_type, parent_id, envelope_id)?;
...
/// (`signing.txt:176-282`) AND the standard `{"response": {"sign_envelope":
/// {…}}}` wrapper, returning `None` when neither is present.
#[must_use]
...
/// Pull the bare `sign_envelopes` array out of a signing list response.
///
/// Tolerates `{"result": true, "sign_envelopes": […]}` and the standard
/// `{"response": {"sign_envelopes": […]}}` wrapper.
#[must_use]
pub fn extract_sign_envelopes(value: &Value) -> Option<&Value> {
    let payload = value.get("response").unwrap_or(value);
    payload.get("sign_envelopes")
}
...
/// and `fields` are ALWAYS a full replacement; `documents` is optional — omit
/// it (`None`) to leave the set unchanged, or supply it for a full declarative
...
pub struct UpdateEnvelopeParams {
    ...
    /// `None` leaves recipients unchanged; `Some(list)` is a full replace.
...
impl UpdateEnvelopeParams {
    ...
    /// Set the full recipient replacement (`None` leaves it unchanged).
    #[must_use]
    ...
    /// Validate the client-side caps that apply to a declarative replace:
    ///
    /// - When `documents` is supplied it must hold 1..=20 entries
    ///   (`signing.txt:361`).
    /// - When `recipients` is supplied it must hold >= 1 entry. Update
    ///   recipients are ALWAYS a FULL REPLACEMENT (`signing.txt:358`), so a
    ///   supplied-but-empty list would wipe every recipient — the contract
    ///   requires >= 1, mirroring the create path's guard.
    ///
    /// A `None` set (leave unchanged) is allowed for each of documents,
    /// recipients, and fields.
    ///
    /// # Errors
    ///
    /// Returns [`CliError::Config`] when a supplied documents list is empty or
    /// exceeds [`MAX_DOCUMENTS`], or when a supplied recipients list is empty.
    pub fn validate(&self) -> Result<(), CliError> {
        ...
        // Update recipients are a full replacement; a supplied-but-empty list
        // would clear the roster. The contract requires >= 1 (signing.txt:358).
    ...
    /// Whether any mutable field was supplied (an empty PATCH is rejected by the
    /// command layer rather than sent).
    #[must_use]
...
///
/// `POST /{parent}/{id}/sign_envelopes/` (JSON body, `signing.txt:288-340`).
/// The caller-side caps are validated first via [`CreateEnvelopeParams::validate`].
pub async fn create_envelope(
    ...
    params.validate()?;
    let path = parent_path(parent_type, parent_id)?;
    client.post_json(&path, &params.to_json()).await
}

/// List envelopes for the parent (offset-paginated, `signing.txt:28`, `:45`).
///
/// `GET /{parent}/{id}/sign_envelopes/`.
...
///
/// `GET /{parent}/{id}/sign_envelopes/{envelope_id}/`.
pub async fn get_envelope(
    ...
    let path = envelope_path(parent_type, parent_id, envelope_id)?;
    client.get(&path).await
}

/// Update mutable fields on a draft envelope (draft-only; non-draft → 403).
///
/// `PATCH /{parent}/{id}/sign_envelopes/{envelope_id}/` (JSON body,
/// `signing.txt:344-362`).
pub async fn update_envelope(
    ...
    params.validate()?;
    let path = envelope_path(parent_type, parent_id, envelope_id)?;
    client.patch_json(&path, &params.to_json()).await
}

/// Soft-delete a draft envelope (terminal envelopes refuse, `signing.txt:31`).
///
/// `DELETE /{parent}/{id}/sign_envelopes/{envelope_id}/`.
...
///
/// `POST /{parent}/{id}/sign_envelopes/{envelope_id}/send/` (no body). A second
...
///
/// `POST /{parent}/{id}/sign_envelopes/{envelope_id}/void/` (JSON body
...
pub async fn void_envelope(
    ...
    let path = envelope_action_path(parent_type, parent_id, envelope_id, "void")?;
...
///
/// `GET /{parent}/{id}/sign_envelopes/{env}/documents/{doc}/download/`. The
...
///
/// `GET /{parent}/{id}/sign_envelopes/{env}/documents/{doc}/signed/download/`.
/// Returns `1609` (HTTP 404) until the document completes (`signing.txt:520`);
/// `403` until the envelope is fully completed.
pub fn signed_document_download_path(
    ...
    let base = envelope_path(parent_type, parent_id, envelope_id)?;
    Ok(format!(
        "{base}documents/{}/signed/download/",
        urlencoding::encode(document_id)
    ))
}
...
///
/// `GET /{parent}/{id}/sign_envelopes/{env}/audit/download/`. The certificate
...
/// 2xx `application/json` success body). Returns `1609` (HTTP 404) until the
/// envelope reaches a terminal state and the certificate is rendered
/// (`signing.txt:531`).
pub fn audit_download_path(
    parent_type: &str,
    parent_id: &str,
    envelope_id: &str,
) -> Result<String, CliError> {
    let base = envelope_path(parent_type, parent_id, envelope_id)?;
    Ok(format!("{base}audit/download/"))
}
...
mod tests {
    ...
    #[test]
    fn parent_path_accepts_workspace_and_org() {
        assert_eq!(
            parent_path("workspace", "4011234567890123456").unwrap(),
            "/workspace/4011234567890123456/sign_envelopes/"
        );
        assert_eq!(
            parent_path("org", "98765432109876543210").unwrap(),
            "/org/98765432109876543210/sign_envelopes/"
        );
    }

    #[test]
    fn parent_path_rejects_other_parent_types() {
        for bad in ["share", "user", "Workspace", "ORG", ""] {
            let err = parent_path(bad, "123").unwrap_err();
            assert!(
                matches!(err, CliError::Config(_)),
                "expected Config error for parent '{bad}', got {err:?}"
            );
        }
    }

    #[test]
    fn parent_path_urlencodes_the_id() {
        assert_eq!(
            parent_path("workspace", "a/b c").unwrap(),
    ...
    #[test]
    fn envelope_path_workspace_vs_org_and_encoding() {
        assert_eq!(
            envelope_path("workspace", "ws1", "env/1").unwrap(),
            "/workspace/ws1/sign_envelopes/env%2F1/"
        );
        assert_eq!(
            envelope_path("org", "org1", "env1").unwrap(),
            "/org/org1/sign_envelopes/env1/"
        );
    }

    #[test]
    fn action_paths_build_send_and_void() {
        assert_eq!(
            envelope_action_path("workspace", "ws1", "env1", "send").unwrap(),
            "/workspace/ws1/sign_envelopes/env1/send/"
        );
        assert_eq!(
            envelope_action_path("org", "org1", "env1", "void").unwrap(),
            "/org/org1/sign_envelopes/env1/void/"
        );
    }
    ...
    fn download_paths_build_and_encode() {
        assert_eq!(
            document_download_path("workspace", "ws1", "env1", "doc-a-b").unwrap(),
            "/workspace/ws1/sign_envelopes/env1/documents/doc-a-b/download/"
        );
        assert_eq!(
            signed_document_download_path("org", "org1", "env1", "doc 1").unwrap(),
            "/org/org1/sign_envelopes/env1/documents/doc%201/signed/download/"
        );
        assert_eq!(
            audit_download_path("workspace", "ws1", "env1").unwrap(),
            "/workspace/ws1/sign_envelopes/env1/audit/download/"
        );
    }

    #[test]
    fn download_paths_reject_bad_parent() {
        assert!(document_download_path("share", "s1", "env1", "doc1").is_err());
        assert!(audit_download_path("nope", "x", "env1").is_err());
    }
    ...
    #[test]
    fn update_validate_rejects_empty_recipient_replace() {
        // A supplied-but-empty recipients list is a FULL REPLACEMENT that would
        // wipe the roster; the contract requires >= 1 (signing.txt:358). A
        // `None` recipients set (leave unchanged) stays allowed.
        let empty = UpdateEnvelopeParams {
            recipients: Some(vec![]),
            ..Default::default()
        };
    ...
    #[test]
    fn update_validate_allows_none_documents() {
    ...
    #[test]
    fn extract_sign_envelopes_list() {
        let v = json!({"result": true, "sign_envelopes": [{"id": "a"}, {"id": "b"}]});
