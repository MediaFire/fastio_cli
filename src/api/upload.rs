...
// These functions differ from the workspace upload helpers above in three ways:
// `action=update` (not `create`), `instance_id` is the File Share id (not a
// workspace id), NO `profile_type`/`folder_id` is sent, and an optional
// `if_version_id` compare-and-swap precondition is honored (File-Share-only;
// 400 on any other target). The share's read gate applies to EVERY step, so
// each helper threads the optional `x-ve-password` header.
...
/// both. Shared by the session-create and single-shot paths so the field set
...
/// state" error). Modeled on [`single_call_upload`]'s multipart mechanics but
/// with the write-back fields and password support, and without the create-only
/// `profile_type` / `folder_id`.
///
...
pub async fn single_shot_fileshare_writeback(
    ...
    // than falling back to a redirect-following client.
    let http_client = reqwest::Client::builder()
    ...
    // Parse the optional password to a sensitive header value ONCE before
    ...
    // `client::build_password_header` seam.
    let password_header = password.map(build_password_header).transpose()?;
    // The multipart text fields derive from the single `writeback_form_fields`
    ...
    // per retry instead of copying the whole buffer.
    ...
    loop {
        ...
        match request.send().await {
            Ok(resp) => {
                ...
                if result_ok {
                    // This DELIBERATELY preserves the top-level `result` (the
            ...
            Err(err) => {
                ...
                if retryable && attempt < CHUNK_MAX_RETRIES {
                    ...
                    eprintln!(
                        ...
                        "warning:".yellow().bold(),
                        attempt,
                        // URL-scrubbed (the write-back path is
...
pub async fn upload_chunk_with_password(
    ...
    // `handle_chunk_response` instead of leaking the password.
    let http_client = reqwest::Client::builder()
        .redirect(reqwest::redirect::Policy::none())
        .timeout(Duration::from_secs(CHUNK_UPLOAD_TIMEOUT_SECS))
        .connect_timeout(Duration::from_secs(CHUNK_CONNECT_TIMEOUT_SECS))
    ...
    // Parse the optional password ONCE before the retry loop; clone the
...
async fn handle_chunk_response(
    ...
    // Shared tolerant parser (text|message, numeric|string-numeric code).
...
///
/// `reqwest::Error`'s `Display` embeds the request URL, which would echo
    ...
    if retryable && *attempt < CHUNK_MAX_RETRIES {
        *attempt += 1;
        eprintln!(
            ...
            "warning:".yellow().bold(),
            *attempt,
            // Scrub the request URL from the error text (see
...
/// Delete an upload session. **The meaning depends on the session's state.**
///
/// `DELETE /upload/{upload_id}/`
///
/// - **Mid-flight session** → aborts the transfer. This is the "cancel" the name
///   refers to, and it discards an incomplete upload.
/// - **Finished session (`stored` / `complete`)** → **reaps the session record
///   only. The stored file is NOT affected.** The web client issues exactly this
///   call on every successful upload, and deliberately swallows `10507`
///   (`UploadIdInvalid`) / `10508` (`UploadIdNotFound`) as the benign race where
///   the session is already gone.
///
/// The name reads as "abort the transfer" in both cases, which has twice been
/// misread as data loss when the call follows a completed upload. It does not
/// delete stored files.
...
/// Maximum length of a whole `relative_path`, in Unicode code points.
pub const RELATIVE_PATH_MAX_CHARS: usize = 8192;

/// Maximum length of a single `relative_path` segment, in Unicode code points.
///
/// A segment is a folder name and carries the same 255-character cap as a
/// filename. The server **shortens** an over-long segment rather than rejecting
/// it, so two paths differing only past this point resolve to the same folder.
pub const SEGMENT_MAX_CHARS: usize = 255;

/// Validate a `relative_path` against the server rules: 1-8192 code points,
/// trailing `/`, no leading `/`, no `..` segments, and no segment over 255
/// code points.
pub fn validate_relative_path(path: &str) -> Result<(), String> {
    if path.is_empty() {
        return Err("relative_path must not be empty".to_owned());
    }
    // Code points, not `len()` (bytes) — the documented cap is characters, so
    // byte-counting would reject a legal path of ~2.7k CJK characters.
    if path.chars().count() > RELATIVE_PATH_MAX_CHARS {
        return Err(format!(
            "relative_path exceeds {RELATIVE_PATH_MAX_CHARS} characters"
        ));
    }
    if !path.ends_with('/') {
    ...
    // Each segment is a folder NAME and is capped individually, in code points.
    //
    // The server REJECTS an over-long segment (never shortens it), naming the
    // offending segment, its length and the limit. Changed by platform commit
    // `46d7ed6dfc` (2026-08-05), the same commit that moved comment bodies and
    // filenames to characters. VERIFIED BY MEASUREMENT 2026-08-08, not
    // just from the published contract: a 256-character segment posted straight
    // to `/upload/batch/` returns per-item `error_code` 146871 — "Folder name in
    // relative_path is 256 characters; the maximum is 255 characters."
    //
    // BEFORE that commit the server SHORTENED instead, measured directly:
    // two paths differing only past character 255 silently resolved to the SAME
    // folder, merging their files. This check predates the fix and was written
    // for that hazard.
    //
    // It stays because it is correct under BOTH regimes and useful under the
    // current one: it turns a round-trip rejection into an immediate local error.
    // Do not remove it as redundant — `chars <= bytes` and reject-here-or-there
    // are not the same user experience.
    for seg in path.split('/').filter(|s| !s.is_empty()) {
        let len = seg.chars().count();
        if len > SEGMENT_MAX_CHARS {
            return Err(format!(
                "relative_path segment exceeds {SEGMENT_MAX_CHARS} characters (got {len}); \
                 the server rejects an over-long segment"
            ));
        }
    }
    Ok(())
...
/// Wrap [`validate_filename`] as a [`CliError`], for the API entry points that
/// name a file server-side.
///
/// Enforced here rather than only in the command layer so that **every** caller
/// — CLI and the embedded MCP server alike — gets the same answer, and so no
/// upload path can quietly bypass it. Without this, `upload text`, chunked
/// session creation, and stream-session creation reached the server unchecked
/// and a too-long name came back as a round-trip `HTTP 406` (`206604`) instead
/// of an immediate, actionable local error.
///
/// # Errors
/// [`CliError::Parse`] carrying [`validate_filename`]'s message.
fn ensure_valid_filename(name: &str) -> Result<(), CliError> {
    validate_filename(name).map_err(CliError::Parse)
}
...
/// Maximum length of the `creator` correlation tag.
///
/// Characters and bytes are interchangeable here **only because the tag is
/// ASCII-only** (enforced before the length check). Do not reuse this constant
/// for a field that admits non-ASCII without switching to `chars().count()`.
pub const CREATOR_TAG_MAX_CHARS: usize = 150;
...
pub fn validate_creator_tag(tag: &str) -> Result<(), String> {
    ...
    // Charset FIRST: the tag is ASCII-only, so once that holds `len()` (bytes)
    // and the character count are identical and the bound below is unambiguous.
    // Checking length first would measure a non-ASCII tag in BYTES and reject a
    // 100-character CJK tag with "must be 1-150 characters" — true that it is
    // rejected, but the wrong reason, which sends the user off fixing length
    // instead of charset.
    ...
    if tag.len() > CREATOR_TAG_MAX_CHARS {
        return Err("creator must be 1-150 characters".to_owned());
    }
    Ok(())
}
...
mod tests {
    ...
    fn relative_path_validation_rules() {
        ...
        // Per-SEGMENT cap: the server shortens an over-long segment instead of
        // rejecting it, so two paths differing only past 255 would silently
        // resolve to the same folder. Fail loudly instead.
        let seg255: String = "s".repeat(SEGMENT_MAX_CHARS);
        let seg256: String = "s".repeat(SEGMENT_MAX_CHARS + 1);
        assert!(validate_relative_path(&format!("{seg255}/")).is_ok());
        assert!(validate_relative_path(&format!("{seg256}/")).is_err());
        // ...including a deep segment, not just the first.
        assert!(validate_relative_path(&format!("ok/{seg256}/deeper/")).is_err());
        // Non-ASCII segments are counted in CODE POINTS: 255 CJK chars is 765
        // bytes and must still be accepted.
        let cjk255: String = "猫".repeat(SEGMENT_MAX_CHARS);
        assert!(validate_relative_path(&format!("{cjk255}/")).is_ok());

        // Whole-path cap is code points too, not bytes: a path built from CJK
        // segments well under 8192 characters must not be rejected for being
        // over 8192 BYTES (the old `len()` check failed at ~2731 characters).
        let long_cjk = format!("{}/", vec!["猫".repeat(200); 20].join("/"));
        assert!(long_cjk.len() > 8192, "fixture must exceed 8192 BYTES");
        assert!(
            long_cjk.chars().count() < RELATIVE_PATH_MAX_CHARS,
            "fixture must be under 8192 CHARS"
        );
        assert!(
            validate_relative_path(&long_cjk).is_ok(),
            "code-point counting must accept a path that is only over-long in bytes"
        );
    }
    ...
    fn filename_validation_rules() {
        // The cap is CHARACTER-counted, not byte-counted — verified live
        // against the API: a 255-char / 757-byte CJK filename uploads
        // successfully, and 256 chars is refused (`206604`).
        ...
        // `ensure_valid_filename` is the API-layer gate; it must agree with the
        // validator and surface a `Parse` error, so no upload path (CLI or MCP)
        // can reach the server with a name the server will reject.
        assert!(ensure_valid_filename(&"x".repeat(255)).is_ok());
        assert!(matches!(
            ensure_valid_filename(&"x".repeat(256)),
            Err(CliError::Parse(_))
        ));
        assert!(ensure_valid_filename(&"猫".repeat(255)).is_ok());
        // Zero-width space.
    ...
    fn creator_tag_validation_rules() {
        ...
        // A short NON-ASCII tag is rejected for its CHARSET, not its length.
        // Before the charset check was moved ahead of the length check, a
        // 100-character CJK tag measured 300 BYTES and was reported as
        // "1-150 characters" — true that it is rejected, wrong reason, and it
        // sends the user off shortening a tag whose length was never the
        // problem.
        let cjk = "猫".repeat(100);
        assert_eq!(cjk.chars().count(), 100);
        assert!(
            cjk.len() > CREATOR_TAG_MAX_CHARS,
            "fixture must exceed 150 BYTES"
        );
        let err = validate_creator_tag(&cjk).expect_err("non-ASCII tag must be rejected");
        assert!(
            err.contains("ASCII"),
            "must be reported as a charset error, got: {err}"
        );
    }
    ...
    async fn network_error_without_url_omits_the_request_url() {
        // The write-back log path is password-bearing, so the rendered
    ...
    fn writeback_send_error_retry_predicate_is_connect_only() {
        // The single-shot write-back is non-idempotent, so the SEND-error
    ...
    fn upload_password_header_accepts_utf8_and_marks_sensitive() {
        // The upload paths reuse the single shared
    ...
    ///
    /// Adapts the `client.rs` one-shot loopback pattern but accepts in a
    ...
    async fn single_shot_writeback_does_not_retry_5xx_terminal_after_one_request() {
        ...
        // A 500 on the single-shot, auto-assembling write-back must be
    ...
    async fn single_shot_writeback_fails_closed_on_redirect_without_following() {
        use std::sync::atomic::Ordering;
        // The no-redirect client must FAIL CLOSED on a 3xx —
