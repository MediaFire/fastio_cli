...
/// Path for uploading or deleting one named asset on an entity.
///
/// `\u{2f}{entity_type}/{entity_id}/assets/{asset_name}/` — uniform across
/// `user`, `org`, `workspace` and `share` (see the published API docs).
#[must_use]
pub fn asset_upload_path(entity_type: &str, entity_id: &str, asset_name: &str) -> String {
    format!(
        "/{}/{}/assets/{}/",
